...
class Fixture:
    def __init__(self) -> None:
        ...
        self.profile = TenantIsolationProfile(
            ...
            namespace_digest=D1,
            rls_policy_digests=(self.policy.evidence_digest,),
            registered_at=110,
        )
        ...
        self.signing_key = InstitutionCryptoKeyReference(
            ...
            key_version=1,
            key_id="kms-signing-v1",
            custody=CryptoKeyCustody.KMS,
            algorithm=CryptoAlgorithm.ED25519,
            public_key_base64url=b64url(pub),
            not_before=100,
            not_after=1000,
            registered_at=100,
        )
...
def test_rls_renderer_is_tenant_and_institution_scoped_and_forced() -> None:
    ...
    assert "modelriskops.institution_id" in ddl
    assert "modelriskops.tenant_id" in ddl
...
def test_isolation_profile_uses_current_rls_and_drift_invalidates_currentness() -> None:
    ...
    stale_new = replace(fx.profile, profile_version=2, registered_at=210)
    with pytest.raises(GovernanceError, match="RLS policy is stale"):
        fx.isolation.register_profile(stale_new)
...
def test_key_rotation_never_rolls_back_to_older_version() -> None:
    ...
    key2 = replace(
        fx.signing_key,
        key_version=2,
        key_id="kms-signing-v2",
        public_key_base64url=b64url(pub2),
        registered_at=200,
        not_before=200,
    )
    fx.keys.register(key2)
    assert fx.keys.current_key("bank-demo", "tenant-a", CryptoKeyPurpose.CONFIG_SIGNING, now=210).evidence_digest == key2.evidence_digest
    fx.keys.transition(CryptoKeyLifecycleState(
        ...
        key_reference_digest=key2.evidence_digest, state_version=2, status=CryptoKeyStatus.RETIRED, effective_at=220,
    ))
...
def test_signed_configuration_chain_is_exact_and_tenant_bound() -> None:
    ...
    request1 = ConfigurationChangeRequest(
        change_id="chg-1", institution_id="bank-demo", tenant_id="tenant-a", sequence=1,
        object_type="tenant-isolation-profile", object_id="tenant-a", previous_configuration_digest=None,
        proposed_configuration_digest=D2, change_reason_digest=D3, requested_by_human_id="security-admin",
        requested_at=120, effective_at=120,
    )
    signed1 = sign_configuration_change(
        request1, previous_change_digest=None, key_reference=fx.signing_key, key_registry=fx.keys,
        signer=fx.signer, signed_at=120, now=120,
    )
    assert verify_signed_configuration_change(signed1, key_registry=fx.keys, now=120) == request1
    registry.append(signed1, key_registry=fx.keys, now=120)

    request2 = ConfigurationChangeRequest(
        change_id="chg-2", institution_id="bank-demo", tenant_id="tenant-a", sequence=2,
        object_type="tenant-isolation-profile", object_id="tenant-a", previous_configuration_digest=D2,
        proposed_configuration_digest=D4, change_reason_digest=D5, requested_by_human_id="security-admin",
        requested_at=130, effective_at=130,
    )
    signed2 = sign_configuration_change(
        request2, previous_change_digest=signed1.evidence_digest, key_reference=fx.signing_key,
    ...
    with pytest.raises(GovernanceError, match="exact tenant change chain"):
        registry.append(replace(signed2, request=replace(request2, sequence=3), previous_change_digest=D1), key_registry=fx.keys, now=140)


def test_future_effective_configuration_does_not_become_current_early() -> None:
    fx = Fixture()
    request = ConfigurationChangeRequest(
        change_id="future", institution_id="bank-demo", tenant_id="tenant-a", sequence=1,
        object_type="tenant-isolation-profile", object_id="tenant-a", previous_configuration_digest=None,
        proposed_configuration_digest=D2, change_reason_digest=D3, requested_by_human_id="security-admin",
        requested_at=120, effective_at=150,
    )
...
def test_encrypted_evidence_binds_tenant_profile_key_and_subject() -> None:
    ...
    envelope = encrypt_governance_evidence(
        plaintext,
        envelope_id="env-1", institution_id="bank-demo", tenant_id="tenant-a", subject_artifact_digest=D2,
        isolation_profile=fx.profile, isolation_registry=fx.isolation,
...
def test_isolation_drift_blocks_new_encryption_and_current_decryption() -> None:
    ...
    with pytest.raises(GovernanceError, match="stale"):
        encrypt_governance_evidence(
            ...
            isolation_profile=fx.profile, isolation_registry=fx.isolation,
            key_reference=fx.encryption_key, key_registry=fx.keys, encryptor=fx.cipher, encrypted_at=210, now=210,
        )
